...
| A7.2 | **Charge LED is common anode** | ✅ **PASSED 2026-09-22.** Diode test, red probe on the common pin, conducts to both others. ⚠️ *A note here once argued that lighting the green die on diode test proved a low-Vf part. It does not: diode-test current is well under a milliamp, where even a high-Vf die sits low. Withdrawn 2026-09-23.* See [`j4-charge-led`](../discovery/findings/j4-charge-led.yaml) |
| A7.3 | **Charge LED green still lights on a flat cell** | ✅ **PASSED 2026-09-22**, measured at the operating point: **0.77 mA at a 3.00 V supply** through 990 Ω, Vf 2.234 V. 🔴 The risk was real: J4 runs from `VOUT`, the cell falling to 3.0 V, and a high-Vf green goes dark exactly when the charge indicator matters. ⚠️ **The check was originally written as "is the die AlGaInP, not InGaN", and that phrasing is a trap**: the chemistry is only a proxy, it cannot be read off a low-current Vf, and the claim that it had been was withdrawn 2026-09-23. **Test the operating point, not the material.** See [`j4-charge-led`](../discovery/findings/j4-charge-led.yaml) |
| A7.4 | **RGB is common anode** | ✅ **PASSED 2026-09-23.** Diode test, red probe on the common pin, conducts to the other three. **Diffused** confirmed by eye. Vf at 5 V: red 1.932, green 2.454, blue 2.607. ⚠️ Those are **2.4-3.1 mA readings and do not contradict the 3.0-3.2 V label**, which is quoted at 20 mA where series resistance dominates. See [`j12-rgb-led`](../discovery/findings/j12-rgb-led.yaml) |
| A7.5 | **Test electret characterised** | ✅ **PASSED 2026-09-23.** Resistance both ways 1.397 k / 1.154 k, asymmetric, so an electret with integral FET. Biased in R51's own circuit, 3.30 V through a measured 2174 Ω: **drain 2.558 V at 339 µA, and RED IS POSITIVE** by measurement rather than convention. 🔴 **Two capsules exist**, and a second uxcell one is unattributed and unbiased. ⚠️ This does **not** establish that it produces signal, which needs E8. See [`bench-test-electret`](../discovery/findings/bench-test-electret.yaml) |
...
✅ **All six A7 checks passed, 2026-09-20 to 2026-09-23.** Every part that could
have cost itself if wrong has been measured on the bench, and no board was
powered to do any of it.

**A7.5 was done first, for the reason it says.** Its job is to tell you whether
the board works, and an unmeasured reference cannot. If E8 reads nothing there
are three suspects: board, capsule, wiring. Measuring it removes one.
**Prove the test article, then use it to prove the board**, exactly as D1 proves
the Seed before it goes near the socket.

🔴 **But "proved" here means proved as far as a bench can, and that is four
things, not five.** A7.5 established that the capsule is genuinely an electret,
which lead is positive *by measurement rather than convention*, its operating
point in R51's own circuit, and a baseline so a later oddity can be told from a
changed capsule. **It did not establish that it produces usable signal**, which
needs this board or a scope. Do not arrive at E8 believing that question is
already answered.

**There are two bench capsules**, and only the small pigtailed one is
characterised. The uxcell has a baseline but its pads are unattributed and it
has never been biased. Having two is useful at E8 precisely because swapping
them separates a board fault from a capsule fault, which one capsule cannot do.
