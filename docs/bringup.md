...
| A7.2 | **Charge LED is common anode** | ✅ **PASSED 2026-09-22.** Diode test, red probe on the common pin, conducts to both others. 🔴 It also lit the **green** die, which a ~2.4 V diode test cannot do to a 3.0 V InGaN part, so it corroborates A7.3 independently. See [`j4-charge-led`](../discovery/findings/j4-charge-led.yaml) |
| A7.3 | **Charge LED green die is AlGaInP, not InGaN** | ✅ **PASSED 2026-09-22: green Vf 2.322 V**, 0.7 V clear of InGaN's 3.0-3.2 V, and it **still lights at a 3.00 V supply** at 0.77 mA, the flat-cell acceptance test. 🔴 The risk was real: J4 runs from `VOUT`, the cell falling to 3.0 V, and an InGaN green goes dark exactly when the charge indicator matters. See [`j4-charge-led`](../discovery/findings/j4-charge-led.yaml) |
